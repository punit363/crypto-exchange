...
export function TradeView({ market }: { market: string }) {
  ...
  const getLookbackWindow = (interval: string) => {
    ...
    switch (interval) {
        case "1m": return now - (1000 * 60 * 60 * 24);      // 1 day
        case "5m": return now - (1000 * 60 * 60 * 24 * 3);  // 3 days
        case "15m": return now - (1000 * 60 * 60 * 24 * 7); // 7 days
        case "1h": return now - (1000 * 60 * 60 * 24 * 30); // 30 days
        case "1d": return now - (1000 * 60 * 60 * 24 * 365);// 1 year
    ...
    const init = async () => {
      ...
      if (chartRef.current) {
        ...
        const chartManager = new ChartManager(
          ...
          ].sort((x, y) => (x.timestamp.getTime() < y.timestamp.getTime() ? -1 : 1)) || [],
          {
            background: "#0e0f14",
            color: "white",
          },
